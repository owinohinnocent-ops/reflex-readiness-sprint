...
function deliveryCard(delivery, { dispatcher = false, rider = false } = {}) {
  const riderLabel = isDemo() ? (delivery.assignedRiderName || "Unassigned") : (delivery.assigned_rider_id ? `Rider profile #${delivery.assigned_rider_id}` : "Unassigned");
  return `<article class="delivery-card"><div class="card-heading"><div><p class="eyebrow">${escapeHtml(deliveryReference(delivery))}</p><h3>${escapeHtml(delivery.customer_name)}</h3></div>${statusBadge(delivery.status)}</div><p class="item">${escapeHtml(delivery.item_description)}</p><dl class="delivery-meta"><div><dt>Phone</dt><dd><a href="tel:${escapeHtml(delivery.customer_phone)}">${escapeHtml(delivery.customer_phone)}</a></dd></div><div><dt>Rider</dt><dd>${escapeHtml(riderLabel)}</dd></div><div><dt>Deliver to</dt><dd>${escapeHtml(delivery.delivery_address)}</dd></div><div><dt>Collect from</dt><dd>${escapeHtml(delivery.pickup_address)}</dd></div></dl>${progress(delivery.status)}<p class="muted">Created ${formatDate(delivery.created_at)}</p>${dispatcher ? assignmentControl(delivery) : ""}${rider ? riderActions(delivery) : ""}</article>`;
}
...
function dispatcherView() {
  const all = activeDeliveries();
  layout(`<div class="page-heading"><div><p class="eyebrow">Operations dashboard</p><h1>Assign work, then follow it through.</h1></div><button id="refresh" class="button ghost">${isDemo() ? "Reset demo" : "Refresh"}</button></div>${statistics()}<section><div class="section-heading"><h2>Delivery requests</h2><span>${all.length} visible</span></div>${isDemo() ? `<p class="integration-note">Demo riders: Brian, Kevin, Samuel, and David. Assignments update only this browser’s temporary demo state.</p>` : `<p class="integration-note">The current backend has no rider-directory endpoint. Enter an <strong>available Rider profile ID</strong> from your operations records to assign an open or failed delivery.</p>`}<div class="delivery-list dispatch-grid">${all.length ? all.map(delivery => deliveryCard(delivery, { dispatcher: true })).join("") : empty("No deliveries found", "Deliveries created by retailers will appear here.")}</div></section>`);
...
async function login(event) {
  ...
  try { const token = await api.login(form.get("phone"), form.get("password")); session = { mode: "real", token: token.access_token, user: await api.getCurrentUser() }; localStorage.setItem(STORE_KEY, JSON.stringify(session)); await refreshDeliveries(); showToast(`Welcome, ${session.user.name}.`); }
...
function bindCommonEvents() { document.querySelector("#logout")?.addEventListener("click", () => { localStorage.removeItem(STORE_KEY); session = null; deliveries = []; demoDeliveries = []; render(); detectApiAvailability(); }); document.querySelector("#refresh")?.addEventListener("click", () => { if (isDemo()) { demoDeliveries = structuredClone(DEMO_SEED); showToast("Demo data reset."); render(); } else refreshDeliveries(); }); }
async function refreshDeliveries({ quiet = false } = {}) { if (!session || isDemo() || syncing) return; syncing = true; render(); try { deliveries = await api.getDeliveries(); lastSynced = new Date(); apiAvailability = "online"; } catch (error) { apiAvailability = "offline"; if (!quiet) showToast(`${friendlyError(error)} You can log out and use Demo Mode.`, "error"); } finally { syncing = false; render(); } }
...
render(); if (session?.mode === "real") refreshDeliveries({ quiet: true }); else detectApiAvailability();
